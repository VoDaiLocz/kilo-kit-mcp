...
export function formatOrchestration(result: OrchestrationResult, format: "markdown" | "json"): string {
  ...
  return [
    ...
    `- Do NOT use native discovery, file, or terminal tools (ListDir, Search, Find, grep_search, view_file, Edit, Bash, run_command, replace_file_content).`,
    `- Use ONLY Kilo-Kit tools: kilo_read_file, kilo_search_files, kilo_grep_code, kilo_write_file, kilo_edit_file, and kilo_run_command.`,
    `- ALWAYS pass sessionId: \`${result.sessionId}\` to every tool call to satisfy Kilo-Sentinel supervision and pre-flight grounding.`,
    "",
    "## Verification Gate",
    verification,
    "",
